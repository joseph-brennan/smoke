...
.. [#f1] http://flask-sqlalchemy.pocoo.org/2.3/api/#flask_sqlalchemy.Pagination
.. [#f2] http://flask-sqlalchemy.pocoo.org/2.3/queries/#querying-records
.. [#f3] http://docs.sqlalchemy.org/en/latest/core/schema.html
"""
...
def paginate(query, schema):
    """A simple helper to paginate a query. [#f1]_

    Breaks apart a query [#f2]_ returned by SQLAlchemy as defined through smoke's SQLAlchemy schema. [#f3]_
    ...
    Parameters:
        query (SQLAlchemy Query): The query to paginate.

        schema (Flask-SQLAlchemy Schema): The ORM schema which defines the objects in the database.
