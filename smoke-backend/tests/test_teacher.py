...
from smoke_backend.models import Teacher


@register
class UserFactory(factory.Factory):

    username = factory.Sequence(lambda n: 'teacher%d' % n)
    email = factory.Sequence(lambda n: 'teacher%d@mail.com' % n)
    password = "mypwd"
...
def test_create_user(client, db, admin_headers):
    ...
    data = rep.get_json()
    user = db.session.query(teacher).filter_by(id=data['user']['id']).first()
